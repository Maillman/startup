...
- [x] Stop Thread from being loaded twice.
...
Found out that the Navigate component was pushing the browser history twice causing the thread page to be loaded twice and would require two back presses to get back to the discussion page. I've since refactored it out so it simply uses the useNavigate hook instead removing some complexity using thread initiating complexity!
